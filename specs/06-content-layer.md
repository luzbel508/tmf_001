...
## 2.3 Confirm final menu categories
...
- **Note:** This is a decision task, not a Copilot task. Resolve it before 2.4.

## 2.4 Populate menu content

- **File(s):** `content/menu-categories.ts`, `content/menu-items.ts`
- **Done when:** Every category from 2.3 has an entry in `MenuCategory[]`, and every real menu item has a corresponding `MenuItem` entry with correct name, description, price (MXN), category reference, and image path. No invented/placeholder menu items remain.
- **Copilot prompt:**
